...
$j = count($Arrlist);
/*for($k=0; $k<$j; $k++){
	$TablaContAforo = cont_aforo($Arrlist[$k], $Alistval[$k]);
	$row_TablaContAforo = mysql_fetch_assoc($TablaContAforo);
	if ($row_TablaContAforo["Contador"] > $row_TablaContAforo["Cantidad"]){
		$Avoid_vent = 1;
		$successGoTo = "error_buy.php?Id_evento=$Id_evento";
	}
} */
...
for($k=0; $k<$j; $k++){
   ...
   begin_ticket($Id_evento, $Id_user, $Arrlist[$k], $Alistval[$k], $refVenta, $Email, $Nom_asistente, $Telefono, $Fecha);
	}
   }
} else { ?>
		<script>
...
$valor = number_format($ValorTkt, 1, '.', '');
...
$Status_User='0';
$Fecha=date("Y-n-j").date(" H:i:s");
begin_pago($Id_user, $Status_User, $Email, $refVenta, $valor, $row_EventoInfo['Nombre'], $Fecha);
//echo $merchantId."-".$refVenta."-".$valor."-".$moneda."-".$signature;

if ($PayMethod=="Payu") { ?>
<body onload="document.formpayu.submit();">

 <form name="formpayu" method="post" action="https://sandbox.checkout.payulatam.com/ppp-web-gateway-payu">
  <input name="merchantId"    type="hidden"  value="<? echo $merchantId; ?>" >
  <input name="accountId"     type="hidden"  value="<? echo $accountId; ?>" >
  <input name="description"   type="hidden"  value="<? echo $row_EventoInfo['Nombre']; ?>"  >
  ...
  <input name="responseUrl"   type="hidden"  value="http://www.evenpot.com/response_test.php?Channel=py" >
  <input name="confirmationUrl"    type="hidden"  value="http://www.evenpot.com/confirm_test.php" >
</form>
<? }
...
?>
<body onload="document.formpaypal.submit();">
<form name="formpaypal" id="paypal_form" action="<? echo $paypal_url; ?>" method="post">
	...
	<input type="hidden" name="item_name" value="<?php echo $row_EventoInfo['Nombre']; ?>">
	...
	<input type="hidden" name="return" value="http://www.evenpot.com/response_pol.php?Channel=pp">
	<input type="hidden" name="cancel_return" value="http://www.evenpot.com/payment-cancelled.html">
	<input type="hidden" name="notify_url" value="http://www.evenpot.com/payments.php">
...
<!--[if gt IE 9]><!-->	<html> <!--<![endif]-->
  ...
  <meta charset="utf-8">
	...
	<link href="/logo.gif" rel="shortcut icon" type="image/vnd.microsoft.icon" />   
...
<div class="container margin_60">
	...
	<div class="col-md-8 add_bottom_15" id="single_tour_desc">
					...
					echo date("d",strtotime($row_TablaFirstLast['Fecha']))." de ".$mesini; 
					}
					else echo date("d",strtotime($row_TablaFirstLast['Fecha'])); 
					
					if ($row_TablaFirstLast['Fecha']!=$row_TablaFirstLast['Fecha_fin'])
					{
						echo " al ".date("d",strtotime($row_TablaFirstLast['Fecha_fin']))." de ".$mesfin." del ".date("Y",strtotime($row_TablaFirstLast['Fecha_fin'])); 
					}
		...
		<div class="table-responsive">
			...
			<tr>
				...
				<td class="text-right">
				 <?php echo "<strong>$".number_format($row_TablaAforoxeve["Valor_tkt"]*$Alistval[$k])."</strong>";
				 $TotalTkts = $Alistval[$k] + $TotalTkts; 
		...
		<? } else { ?>
			...
			<div class="step">
				...
				<? if(trim($Rows_TablaReg["Entidad"]) == 1){	?>	
					<div class="col-md-6 col-sm-6">
						<div class="form-group">
							...
							<input type="text" name="Entidad" id="Entidad" value="<?php echo $row_TablaUs['Institution'];?>" class="form-control" placeholder="Entidad/Institucion" tabindex="7" required oninvalid="this.setCustomValidity('Debe llenar este campo')" oninput="setCustomValidity('')" />
						</div>
					</div>
					<? } ?>
					...
					<div class="col-md-6 col-sm-6">
						<div class="form-group">
							...
							<input type="text" name="Cargo" id="Cargo" value="<?php echo $row_TablaUs['Profesion'];?>" class="form-control" placeholder="Cargo/Profesion" tabindex="8" required oninvalid="this.setCustomValidity('Debe llenar este campo')" oninput="setCustomValidity('')"/>
						</div>
					</div>
					<? } ?>
				</div>
				...
				<div class="row">
					<div class="col-md-6 col-sm-6">
						...
						<select class="form-control"  name="Pais"  tabindex="9" required oninvalid="this.setCustomValidity('Seleccione un Pais')" oninput="setCustomValidity('')">
					...
					<div class="col-md-6">
						<div class="form-group">
							<label>Ciudad</label>
							<input type="text" name="Ciudad" id="Ciudad" class="form-control" value="<?php echo $row_TablaUs['Ciudad_nac'];?>" placeholder="Ciudad" tabindex="10" required oninvalid="this.setCustomValidity('Debe llenar este campo')" oninput="setCustomValidity('')"/>
						</div>
					</div>
				...
				<div class="row">
					...
					<div class="col-md-3">
						<div class="form-group">
							<label>Genero</label>
							<select name="Genero" id="Genero" tabindex="11" class="form-control" required oninvalid="this.setCustomValidity('Seleccione su Genero')" oninput="setCustomValidity('')">
					...
					<div class="col-md-3">
						<div class="form-group">
							...
							<input name="Birthday" type="text" value="<?php echo str_replace("-", "/", date("d-m-Y", strtotime($row_TablaUs['Birthday'])));?>" data-date-format="dd/mm/yyyy" class="date-pick form-control" placeholder="dd/mm/yyyy" tabindex="12" required oninvalid="this.setCustomValidity('Debe llenar este campo')" oninput="setCustomValidity('')"/>
						</div>
					</div>
					<? } ?>
				</div><!--End row -->
			...
			<div class="row">
					<div class="col-md-6 col-sm-6">
						<input type="radio" name="PayMethod" value="Payu" class="attrInputs"><img src="img/cards.png" width="267" height="43" alt="Cards" class="cards">
					</div>
			</div>
			<hr>
...
<div id="recfollow" class="box_style_1">
	<? 
	if(isset($_SESSION['Id_user'])){
		if($Rows_FollxEve==1) 
